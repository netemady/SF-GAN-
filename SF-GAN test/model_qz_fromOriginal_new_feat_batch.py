...
class graph2graph(object):
    def __init__(self, sess, test_dir, train_dir, image_size, output_size,
                 batch_size, L1_lambda2, Setting_value, sample_size=1,
                 gf_dim=5, df_dim=5, L1_lambda=1e7,
    ...
    def build_model(self):
        ...
        self.fake_B, self.d3, self.d2_w = self.generator(self.real_A, self.node, self.features)
    ...
    def train(self, args, sc, fc, sc_test, fc_test, ff1_new_train, ff1_new_test, lambda2, setting_value):
        ...
        for epoch in xrange(args.epoch):
            ...
            # Run g_optim twice to make sure that d_loss does not go to zero (different from paper)
                ...
                errD_fake = self.d_loss_fake.eval({self.real_data: batch_images, self.node: batch_nodes, self.features: feat_train3})
                errD_real = self.d_loss_real.eval({self.real_data: batch_images, self.node: batch_nodes, self.features: feat_train3})
                errG = self.g_loss.eval({self.real_data: batch_images, self.node: batch_nodes, self.features: feat_train3})
            ...
            for i, sample_image in enumerate(sample_images):
                ...
                if i < batch_idxs_test:
                    ...
                    errD_real_test = self.d_loss_real.eval(
                        {self.real_data: sample_images[i], self.node: sample_nodes[i], self.features: feat_test3})
                    errG_test = self.g_loss.eval({self.real_data: sample_images[i], self.node: sample_nodes[i], self.features: feat_test3})
            ...
            '''if epoch%20==0 or epoch==args.epoch-1:
              plt.subplot(211)
              plt.plot(total_loss_train, '--r', total_loss_test, 'bs')
              plt.legend(['train','test'])
              plt.ylabel('avg loss per sample')
              plt.xlabel('epoch')
              plt.ylim(0, 0.5)
              plt.title('Setting3, L1_lambda2 = {}'.format(self.L1_lambda2))


              plt.subplot(212)
              plt.plot(pearsonCorrelation_train, '--r', pearsonCorrelation, 'bs')
              plt.legend(['train', 'test'])
            ...
            if epoch%200==0 or epoch==args.epoch-1:
                ...
                if epoch==args.epoch-1:
                  s = "finalWeights_set%s_lm%s" % (setting_value, lambda2) #for last epoch
                  ss = "finalWeights_vs_epochs_set%s_lm%s" % (setting_value, lambda2)
                  vars = tf.trainable_variables()
                  vals = self.sess.run(vars)
                  value_ = vals[16]
                  #v1 = value_[0, :, 0, 11:]
                  ...
                  np.save(s, v1)
                  np.save(ss, v1_final)
                #v1_mean = np.mean(np.absolute(v1))
                #print(v1_mean)

                best = errG

        s1 = "total_loss_test_set%s_lm%s" % (setting_value, lambda2)
        s2 = "total_loss_train_set%s_lm%s" % (setting_value, lambda2)
        s3 = "pearsonCorrelation_test_set%s_lm%s" % (setting_value, lambda2)
        s4 = "pearsonCorrelation_train_set%s_lm%s" % (setting_value, lambda2)
        s5 = "r2_score_test_set%s_lm%s" % (setting_value, lambda2)
        s6 = "mse_test_set%s_lm%s" % (setting_value, lambda2)
        s7 = "r2_score_train_set%s_lm%s" % (setting_value, lambda2)
        s8 = "mse_train_set%s_lm%s" % (setting_value, lambda2)

        '''s5 = "data_RESTINGSTATE_Partial_L2_set%s_lm%s" % (setting_value, lambda2)

        adict = {}
        adict[s1] = total_loss_test
        adict[s2] = total_loss_train
        adict[s3] = pearsonCorrelation
        adict[s4] = pearsonCorrelation_train
        sio.savemat(s5+'.mat', adict)

        np.save(s1, total_loss_test)
        np.save(s2, total_loss_train)
        np.save(s3, pearsonCorrelation)
        np.save(s4, pearsonCorrelation_train)
        
        np.save(s5, r2_score_value_test)
        np.save(s6, mse_value_test)
        np.save(s7, r2_score_value_train)
        np.save(s8, mse_train)'''
    ...
    def generator(self, image, node, features,  y=None):
        nn = 161
        with tf.variable_scope("generator") as scope:
            ...
            return tf.reshape(tf.stack(out_list), [len(out_list), self.image_size[0], self.image_size[0], 1]), self.d3, self.d2_w
