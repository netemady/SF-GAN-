...
class graph2graph(object):
    def __init__(self, sess, test_dir, train_dir, image_size, output_size,
                 ...
                 gf_dim=5, df_dim=5, L1_lambda=1e2,
    ...
    #batch_loss_test = []
            ...
            for i, sample_image in enumerate(sample_images):
                ...
                if i < batch_idxs_test:
                  ...
                  errD_fake_test = self.d_loss_fake.eval({self.real_data: sample_images[i], self.node: sample_nodes[i]})
                  errD_real_test = self.d_loss_real.eval({self.real_data: sample_images[i], self.node: sample_nodes[i]})
        ...
        '''np.save("total_loss_test_nofeat", total_loss_test)
        np.save("total_loss_train_nofeat", total_loss_train)
        np.save("pearsonCorrelation_test_nofeat", pearsonCorrelation)
        np.save("pearsonCorrelation_train_nofeat", pearsonCorrelation_train)
        np.save("mse_test_nofeat", mse_value_test)
        np.save("r2_score_test_nofeat", r2_score_value_test)
        

        np.save("mse_test_nofeat", mse_value_test)
        np.save("mse_train_nofeat", mse_train)

        np.save("r2_score_train_nofeat", r2_score_value_train)
        np.save("r2_score_test_nofeat", r2_score_value_test)

        s5 = "data_RESTINGSTATE_Partial_L2_nofeat"

        adict = {}
        adict["total_loss_test_RESTINGSTATE_partial_L2_nofeat"] = total_loss_test
        adict["total_loss_train_RESTINGSTATE_partial_L2_nofeat"] = total_loss_train
        adict["pearsonCorrelation_test_RESTINGSTATE_partial_L2_nofeat"] = pearsonCorrelation
        adict["pearsonCorrelation_train_RESTINGSTATE_partial_L2_nofeat"] = pearsonCorrelation_train
