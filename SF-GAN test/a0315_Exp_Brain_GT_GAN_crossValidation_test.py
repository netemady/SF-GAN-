...
import scipy.misc
import numpy as np
from model_GTGAN_crossValidation_test import graph2graph
...
set1_allFolds = np.load(s1)
set2_allFolds = np.load(s2)
set3_allFolds = np.load(s3)'''

#Lambda2 = [1e0, 1e1, 1e2, 1e3, 1e4, 1e5, 1e6, 1e7]#
Lambda2 = [1]
epoch_set = [40, 80, 110, 135, 150]
settings_vals = [1]
...
for setting_value in settings_vals:
    for lambda2 in Lambda2:
        ...
        for file in files:
            ...
            fc_test = allSC[list(range(0, test_size))]
