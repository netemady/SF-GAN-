...
import scipy.misc
import numpy as np
from model_GTGAN_sensitivity_plots import graph2graph
...
def hyperparams(lambda1, lambda2, setting_value):
    ...
    parser.add_argument('--L1_lambda', dest='L1_lambda', type=float, default=lambda1, help='weight on L1 term in objective')
...
#dataset_lm_sett = np.array([[1e3, 1e3, 1e3], [1e2, 1e2, 1e3], [1e3, 1e5, 1e6], [1e6,1e6,1e6], [1e4,1e4,1e4], [1e1,1e3,1e1], [1e3,1e2,1e3]])
#lambda1_values = [1e5, 1e4, 1e3]
lambda1_values = [1e2]

l1 = range(0,1001,100)
l2 = [5e3, 1e4, 5e4, 1e5, 5e5, 1e6, 5e6, 1e7]
   ...
   for lambda1 in lambda1_values:
     ...
     for lambda2 in lambda2_values:
            ...
            with tf.Session() as sess:
                    ...
                    model.train(args, sc_train, fc_train, sc_test, fc_test, ff1_new_train, ff1_new_test, lambda2,
                                     setting_value)
                    ####################################################
     ...
     '''s_label = "lambda1=%s" %lambda1
     if lambda1 == 1e5:
         plt.plot(lambda2_values, results_corr[:,setting_value - 1], "-b", label=s_label)
     elif lambda1 == 1e4:
         plt.plot(lambda2_values, results_corr[:, setting_value - 1], "-r", label=s_label)
     elif lambda1 == 1e3:
         plt.plot(lambda2_values, results_corr[:, setting_value - 1], "-g", label=s_label)
     else:
         plt.plot(lambda2_values, results_corr[:, setting_value - 1], "-m", label=s_label)'''


     print(np.transpose(results_corr[:,setting_value - 1]))
