...
def conv2d(input_, output_dim, k_h=5, k_w=5, d_h=3, d_w=3, stddev=0.02, name="conv2d"):
    with tf.variable_scope(name):
...
def e2e(input_, output_dim, k_h, k_w, d_h=1, d_w=1, stddev=0.02, name="e2e"):
    with tf.variable_scope(name):
        w1 = tf.get_variable('w1', [k_h, k_w, input_.get_shape()[-1], output_dim],
                            initializer=tf.truncated_normal_initializer(stddev=stddev))
        ...
        w2 = tf.get_variable('w2', [k_h,k_w, input_.get_shape()[-1], output_dim],
                            initializer=tf.truncated_normal_initializer(stddev=stddev))
...
def e2n(input_, output_dim, k_h, k_w, d_h=1, d_w=1, stddev=0.02, name="e2n"):
     with tf.variable_scope(name):
        w = tf.get_variable('w', [1, k_w, input_.get_shape()[-1], output_dim],
                            initializer=tf.truncated_normal_initializer(stddev=stddev))
...
def n2g(input_, output_dim, k_h, k_w, d_h=1, d_w=1, stddev=0.02, name="e2n"):
     with tf.variable_scope(name):
        w = tf.get_variable('w', [k_w,1, input_.get_shape()[-1], output_dim],
                            initializer=tf.truncated_normal_initializer(stddev=stddev))
        conv = tf.nn.conv2d(input_, w, strides=[1, d_h, d_w, 1], padding='VALID')
        biases = tf.get_variable('biases', [output_dim], initializer=tf.constant_initializer(0.0))
        conv = tf.reshape(tf.nn.bias_add(conv, biases), conv.get_shape())
...
def de_n2g(input_, output_shape, k_h, k_w, d_h=1, d_w=1, stddev=0.02, name="de_n2g", with_w=False):
    with tf.variable_scope(name):
...
def de_e2n(input_, output_shape, k_h, k_w, d_h=1, d_w=1, stddev=0.02, name="de_n2g", with_w=False):
    with tf.variable_scope(name):
        # filter : [height, width, output_channels, in_channels]
...
'''def de_e2n(input_, output_shape, k_h, k_w, d_h=1, d_w=1, stddev=0.02, name="de_n2g", with_w=False):
    with tf.variable_scope(name):
        # filter : [height, width, output_channels, in_channels]
        w1 = tf.get_variable('w1', [1, 1, output_shape[-1], input_.get_shape()[-1]],
                            initializer=tf.random_normal_initializer(stddev=stddev))
...
def de_e2e(input_, output_shape, k_h, k_w, d_h=1, d_w=1, stddev=0.02, name="de_n2g", with_w=False):
    with tf.variable_scope(name):
        # filter : [height, width, output_channels, in_channels]
...
def deconv2d(input_, output_shape, k_h=5, k_w=5, d_h=3, d_w=3, stddev=0.02, name="deconv2d", with_w=False):
    with tf.variable_scope(name):
        # filter : [height, width, output_channels, in_channels]
